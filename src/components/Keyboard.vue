<template>
  <div>
    {{width}}
    <svg :width="width" :height="height" style="shape-rendering: crispEdges">
      <path
        :d="path.path"
...
export default {
  ...
  data() {
    ...
    for (let note = this.start; note < this.start + this.length; note++) {
      ...
      if (shape.type === 0) {
        ...
        if (shape.right && shape.left) {
          ...
          path += `h ${NATURAL_WIDTH - SHARP_WIDTH} v ${SHARP_HEIGHT} h ${
            SHARP_WIDTH / 2
          } `;
          ...
          path += `h ${SHARP_WIDTH / 2} v ${-SHARP_HEIGHT} `;
          path += `h ${
            NATURAL_WIDTH - SHARP_WIDTH / 2
          } v ${NATURAL_HEIGHT} h ${-NATURAL_WIDTH}`;
        } else {
          path = `M ${position} ${NATURAL_HEIGHT + 1} v ${-NATURAL_HEIGHT} `;
    ...
    console.log('natural', NATURAL_WIDTH)
    return {
      keys,
